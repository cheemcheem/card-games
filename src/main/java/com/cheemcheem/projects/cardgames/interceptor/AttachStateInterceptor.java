...
public class AttachStateInterceptor implements HandlerInterceptor {
  ...
  // list of endpoints that require game id to be attached to request attributes
  private static final Predicate<String> excludedPaths = Pattern.compile("(/api/game/details)"
      + "|(/api/game/details/static)"
      + "|(/api/game/delete)"
      + "|(/api/game/exit)"
      + "|(/api/hand/get)"
      + "|(/api/game/start)"
